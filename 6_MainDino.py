...
def generate_embeddings(batch_size=16):
    ...
    np.save("embeddings_dino.npy", all_embeddings)
    with open("image_paths_dino.txt", "w") as f:
        for path in image_files:
            f.write(path + "\n")
    print(f"Zapisano {len(all_embeddings)} embeddingów.")
# Generowanie lub ładowanie embeddingów
if not (os.path.exists(
        "embeddings_dino.npy") and os.path.exists("image_paths_dino.txt")):
...
with open("image_paths_dino.txt", "r") as f:
