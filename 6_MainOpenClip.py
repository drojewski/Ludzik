...
# Załaduj model OpenCLIP z lepszym modelem ViT-L-14-336px
...
def generate_embeddings(batch_size=16):
    ...
    np.save("embeddings_large.npy", all_embeddings)
    with open("image_paths_large.txt", "w") as f:
...
if not (os.path.exists("embeddings_large.npy")):
    print("Generowanie embeddingów z modelem ViT-L-14-336 OpenCLIP...")
    generate_embeddings()
else:
    print("Ładowanie istniejących embeddingów i ścieżek...")

embeddings = np.load("embeddings_large.npy")
with open("image_paths_large.txt", "r") as f:
...
results = search_similar_with_classification(query_img, k=50, threshold=0.5)
